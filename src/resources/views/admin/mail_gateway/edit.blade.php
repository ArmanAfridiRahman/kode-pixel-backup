	...
	<div class="i-card-md">
		...
		<div class="card-body">
			<form action="{{route('admin.mailGateway.update')}}" class="add-listing-form" enctype="multipart/form-data" method="post">
				@csrf
				<input type="hidden" name="id" value="{{$gateway->id}}" id="">
					<div class="row">					
						@foreach ($gateway->credential as $k=>$v)
							...
							@foreach($v  as $subKey=>$subVal)
								<div class="col-lg-6">
									<div class="form-inner">
										<label for="{{ $subKey }}">{{ ucfirst(str_replace('_',' ', $subKey)) }}
											<small class="text-danger">*</small>
										</label>
	...
	<div class="modal fade" id="test" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
			<div class="modal-content">
				...
				<form action="{{route('admin.mailGateway.test')}}" id="" method="post" enctype="multipart/form-data">
					@csrf
...
<script>
	(function($){
       	"use strict";
		$(".select2").select2({
			placeholder:"{{translate('Select Security')}}",
		})
	})(jQuery);
</script>
@endpush
