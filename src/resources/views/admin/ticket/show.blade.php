                    ...
                    <div class="ticket-conversation">
                        ...
                        @else
                            <form action="{{ route('admin.ticket.reply') }}" class="give-replay" method="post">
                                @csrf
                                <input hidden value="{{ $ticket->id }}" type="text" name="id" id="">
                            ...
                            <div class="i-card-md">
                                ...
                                <div class="card-body px-0 pb-0">
                                    @forelse($messages    as $message)
                                        @php
                                            $imgUrl = imageUrl(config('settings')['file_path']['profile']['user']['path'] . '/' . @$ticket->user->file->name, @$ticket->user->file->disk);

                                            if ($message->admin_id) {
                                                $imgUrl = imageUrl(config('settings')['file_path']['profile']['admin']['path'] . '/' . @$message->admin->file->name, @$message->admin->file->disk);
                                            }
                                        @endphp
                                        <div class="message-item d-flex justify-content-between align-items-start">
                                            <div class="d-flex flex-grow-1">
                                                <div class="author-image me-3">
                                                    <img class="rounded-circle avatar-sm" src="{{ $imgUrl }}" alt />
                                                </div>
                                            ...
                                            <div class="mesg-action">
                                                <div class="i-dropdown">
                                                    <a href="#" class="icon-btn info p-2" type="button"
